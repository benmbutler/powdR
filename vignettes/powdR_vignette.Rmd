...
As mentioned previously, an alternative to minimisation of an objective function (the default for `fps()`) in non-negative least squares (NNLS). This is implemented in `powdR` version 0.2.0, and provides very fast computation. Whilst NNLS can struggle with X-ray amorphous phases, its speed makes it particularly useful fast quantititative estimates of a sample using a whole reference library at once.

```{r, fig.show='hold', fig.cap = "**Figure 4:** Application of NNLS to the sandstone soil", message = FALSE, warning = FALSE}

#Apply fps to the sandstone sample using NNLS. Note that when NNLS is selected as the solver argument, there is no need to define the refs because all phases in the library are used
fit <- fps(lib = minerals,
           smpl = soils$sandstone,
           solver = "NNLS",
           std = "QUA.1",
